...
import { useGetDelhiParticlesQuery,useGetKeralaParticlesQuery,useGetMumbaiParticlesQuery } from "state/api";
import SliderProton from "components/SliderProton";

const MumbaiParticles = () => {
  const [selectedHours, setSelectedHours] = useState([0, 24]);
  const handleChangeHours = (event, value) => {
    setSelectedHours(value);
  };
  const { data } = useGetMumbaiParticlesQuery();
  ...
  return (
    <Box m="1.5rem 2.5rem">
      <Header title="Mumbai Location All air impurity particles Comparision" subtitle="Chart of PM particles (µg/m³) vs Hours" />
      <Box height="5vh">
...
export default MumbaiParticles;
